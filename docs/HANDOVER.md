...
| 推し・推しの棚 | 推しは複数選べる（グッズページの顔チップ / メンバーページのトグル。ヘッダーのボタンは外した）。グッズページ上部にメンバーごとの棚（全アイテム＋カートへ）がまとまり、最後に選んだ子の色が下線・リボン・番号・TimeTree の枠になる。文字色はグレーのまま。localStorage `sn-oshi`（id の配列）に保存（`assets/js/oshi.js`） |
