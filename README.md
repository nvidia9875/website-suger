...
推しは**複数**選べます。選ぶ場所は、ヘッダーの「推し」ボタン（ダイアログ）、メンバーページの「◯◯を推しにする」、グッズページ上部の顔チップの3つ。選んだ子は次のように扱われます。
...
仕組み: `assets/js/oshi.js` が `localStorage`（キー `sn-oshi`、メンバー id の配列・選んだ順）に保存し、`<html>` に `data-oshi` と CSS 変数 `--oshi`（AA を満たす濃色）/ `--oshi-soft`（本来の色）を当てる。CSS は `var(--oshi, var(--ink))` のように書き、未設定ならモノトーンに落ちる。カード・メンバーページの「その子自身の色」は `--own` を要素ごとに当てる。変更は `sn:oshi` イベントで各ページに伝わる。選択ダイアログは JS が組み立てるので HTML には無い。この端末にだけ保存され、どこにも送られません。`goods.html?m=<id>` で開くとその子を推しに追加して棚を出す（メンバーページの「すべて見る」から使う）。
